...
int light_pins[max_pins] = {  // first channel is the LAST one. goes right to left
...
int strobe(unsigned long wait)
{
  ...
  //return(0);
    ...
    x=255;
    while((millis() - time) < cycle_delay) {   //// FIREPLACE!!
      read_controls();
        for(st = 0; st < max_pins; ++st) {  // 
          if(pin_brightness[st] != 255)         //FIREPLACE
            analogWrite(light_pins[st],x);
        } // for st
        delay(cycle_delay/256);
        if((--x) < 0) x = 0;
    }  // while time wait
